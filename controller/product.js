...
module.exports.addProduct = async (req, res) => {
	if (typeof req.body == undefined) {
		...
		try {
			...
			const cat = await Category.findOne({ _id: ObjectID(req.body.category._id) });
			if (!cat) {
				return res.json({
			...
			const fileName = `${Date.now()}_${Math.floor(Math.random() * 300000 + 200000)}.${imageType}`;
			fs.writeFileSync(localPath + fileName, base64Data, 'base64', function (err, data) {
				if (err) {
					throw new Error(err);
				}
			});
...
module.exports.editProduct = async (req, res) => {
	...
	if (typeof req.body == undefined || req.params.id == null) {
		...
		try {
			...
			await Product.updateOne({ _id: req.params.id }, { $set: { ...productInputValue, image: `/files/${fileName}` } });
